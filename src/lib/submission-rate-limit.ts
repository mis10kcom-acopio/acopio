...
const COOKIE_NAME = "has_submission_rl";
const MAX_SUBMISSIONS_PER_HOUR = 10;
const WINDOW_MS = 60 * 60 * 1000;

export const SUBMISSION_RATE_LIMIT_MESSAGE =
  "Has superado el límite de 10 reportes por hora. Por favor, intenta de nuevo más tarde para evitar saturar el sistema.";
...
/** Comprueba límite por cookie firmada + IP del request (Vercel headers). */
export async function assertSubmissionRateLimit(): Promise<
...
> {
  ...
  if (payload.ts.length >= MAX_SUBMISSIONS_PER_HOUR) {
    return { ok: false, message: SUBMISSION_RATE_LIMIT_MESSAGE };
  }

  return { ok: true };
}

/** Registra un envío exitoso en la cookie de rate limit. */
export async function recordSuccessfulSubmission(): Promise<void> {
  const headerStore = await headers();
  const ipHash = hashClientIp(getClientIp(headerStore));
  const payload = await readPayload(ipHash);
